...
test("shows protected land and calculates a midyear depreciation period", async () => {
  const calls: string[] = [];
  let finishInitialPeriods: ((response: Response) => void) | undefined;
  const initialPeriods = new Promise<Response>((resolve) => { finishInitialPeriods = resolve; });
  vi.spyOn(globalThis, "fetch").mockImplementation((input) => {
    ...
    if (url === "/api/assets/years/1/periods") return initialPeriods;
  ...
  finishInitialPeriods?.(new Response("[]", { status: 200, headers: { "Content-Type": "application/json" } }));
  await waitFor(() => expect(screen.getAllByText("120.92 €")).toHaveLength(2));
